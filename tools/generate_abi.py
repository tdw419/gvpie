...
def generate_wgsl_abi(glyph_atlas):
    ...
    wgsl_content += "    switch char_code {\n"

    for char, data in glyph_atlas.items():
        char_code = ord(char)
        wgsl_content += f"        case {char_code}u: {{\n"
        wgsl_content += "            switch row {\n"
        for i, row_data in enumerate(data):
            wgsl_content += f"                case {i}u: {{ return {row_data}u; }}\n"
        wgsl_content += "                default: {{ return 0u; }}\n"
        wgsl_content += "            }\n"
        wgsl_content += "        }}\n"
